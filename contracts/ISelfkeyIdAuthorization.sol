...
interface ISelfkeyIdAuthorization {
    ...
    function getMessageHash(address _from, address _to, uint256 _amount, string memory _scope, uint _timestamp) external view returns (bytes32);

    function getEthSignedMessageHash(bytes32 _messageHash) external view returns (bytes32);

    function verify(address _from, address _to, uint256 _amount, string memory _scope, uint _timestamp, address _signer, bytes memory _signature) external view returns (bool);
